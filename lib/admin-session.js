...
function createSessionToken(passwordHash) {
...
export async function isAuthenticated() {
  ...
  return safeEqual(session, createSessionToken(getAdminPasswordHash()));
}

export async function login(password) {
  const hash = getAdminPasswordHash();
  if (!hasConfiguredAdminPassword() || !(await bcrypt.compare(password, hash))) {
    return false;
  }

  const cookieStore = await cookies();
  cookieStore.set(COOKIE_NAME, createSessionToken(hash), {
    httpOnly: true,
    sameSite: "lax",
